        ...
        <div class="row">
            ...
            <div class="col-lg-2 order-lg-1 order-2">
                <ul class="image_list">
                    ...
                    <li data-image="{{asset($image->large_image)}}">
                        <img src="{{asset($image->medium_image)}}" alt="Image">
                    </li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="col-lg-5 order-3">
                <div class="product_description">
                    <div class="product_category">{{$mainCategory->category_name}} / {{$subCategory->sub_category_name}}<span style="float: right"> <strong>View : {{$product->view_total}}</strong></span></div>
                    <div class="product_name">৳ {{$product->sale_Price}}</div>
                    <h4>Deal Code : {{$product->id}}</h4>
                    <style>
                        .checked {
                          color: orange;
                        }
                        ...
                        {{Form::open(['url'=> '/add-to-cart', 'method' => 'post', 'class' => 'form-horizontal' ])}}
                            <div class="clearfix" style="z-index: 1000;">
                                <div class="row">
                                    ...
                                    <div class="col-6">
                                        <p style="margin: 0; padding: 0;">Size : {{$product->size}}</p>
                                        <p style="margin: 0; padding: 0;">Others : {{$product->others}}</p>
                                    </div>
                                    <!-- Product Color -->
                                </div>
                            </div>
                            <!-- Product Quantity -->
                            <br/>
                            <div class="product_category"><span style="font-size: 20px; font-weight: 600">৳ {{$product->product_price_eng}} </span> <strong> | </strong> {{ $product->note_eng}} </div><br />
                            ...
                            <div class="button_container">
                                ...
                                <a href="{{URL::to('/cart')}}" class="button cart_button" style="background-color: #0a8f08; font-size: 12px">Cart Page</a>
                            </div>
                            {{Form::close()}}
                    </div>
                </div>
                    ...
                    <div class="card-body" style="background-image: linear-gradient(0deg,#fff 0%,#17a2b8);">
                        <div style="text-align: center">
                            ...
                            <div class="row">
                                ...
                                <div class="col-2"><strong>:</strong></div>
                                <div class="col-5">{{$shopper->shipping_info}} {{$shopper->id}}</div>
                            </div>
                            <br/>
                            <div class="row">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        @foreach($reletedProducts as $reletedProduct)
                            <div class="row mb-2">
                                <div class="col-7">
                                    <a href="{{URL::to('/product-view-by-id/'.$reletedProduct->id)}}"><h5>{{ str_limit($reletedProduct->product_name_eng, 40) }}</h5></a>
                                    <h4>৳ {{$reletedProduct->product_price_eng}}</h4>
                                    <a href="{{URL::to('/product-view-by-id/'.$reletedProduct->id)}}" class="btn btn-info">add to cart</a>
                                </div>
